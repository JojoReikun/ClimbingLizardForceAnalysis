function varargout = Nano17_recalibrator(varargin)
% NANO17_RECALIBRATOR MATLAB code for Nano17_recalibrator.fig
%      NANO17_RECALIBRATOR, by itself, creates a new NANO17_RECALIBRATOR or raises the existing
%      singleton*.
%
%      H = NANO17_RECALIBRATOR returns the handle to a new NANO17_RECALIBRATOR or the handle to
%      the existing singleton*.
%
%      NANO17_RECALIBRATOR('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in NANO17_RECALIBRATOR.M with the given input arguments.
%
%      NANO17_RECALIBRATOR('Property','Value',...) creates a new NANO17_RECALIBRATOR or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before Nano17_recalibrator_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to Nano17_recalibrator_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help Nano17_recalibrator

% Last Modified by GUIDE v2.5 05-Sep-2018 12:43:50

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @Nano17_recalibrator_OpeningFcn, ...
                   'gui_OutputFcn',  @Nano17_recalibrator_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before Nano17_recalibrator is made visible.
function Nano17_recalibrator_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to Nano17_recalibrator (see VARARGIN)

% Choose default command line output for Nano17_recalibrator
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes Nano17_recalibrator wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = Nano17_recalibrator_OutputFcn(hObject, eventdata, handles) 
...
% handles.time=1:length(handles.Fx) as defined above;
...
% --- SAVING CORRECTED RAW FORCE DATA:
